...
import MagicMoodboard, { type MoodboardImage } from '../../components/MagicMoodboard'
...
export default function MagicMovement() {
  ...
  const addMoodboardReference = (image: MoodboardImage) => {
    setReferences(prev => {
      if (prev.some(ref => ref.url === image.url)) return prev
      return [{
        id: `mood_${Date.now()}`,
        url: image.url,
        name: image.label,
    ...
    if (!prompt.trim() && image.prompt) setPrompt(image.prompt)
    showToast('Added moodboard concept to references', 'success')
  }
  ...
  const generate = async () => {
    ...
    try {
      ...
      const primaryUrl = aiUrl || fallback
      ...
      setTimeout(() => {
        ...
        setIsGenerating(false)
        showToast(
          aiUrl ? 'Design ready' : 'AI unavailable â€” showing a placeholder. Check API keys and restart the server.',
          aiUrl ? 'success' : 'error',
        )
      }, 350)
    } catch {
    ...
    <div className="bb-page" data-testid="magic-movement" style={{ position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div
          ...
          style={{
            display: 'grid',
            gridTemplateColumns: 'minmax(300px, 0.75fr) minmax(0, 1.1fr) minmax(280px, 0.8fr)',
        ...
        >
          ...
          <MagicMoodboard
            ...
            lunaBrief={intakeDNA?.notes || prompt || null}
            onPick={addMoodboardReference}
          />
        </div>
      </div>
    </div>
